...
METHOD_ODDS_EXPECTED_WINDOW_HOURS = max(
    _safe_float_env("METHOD_ODDS_EXPECTED_WINDOW_HOURS", "48"), 0.0
)
...
POLYMARKET_MARKET_INFO_MAX_ATTEMPTS = min(
    max(_safe_int_env("POLYMARKET_MARKET_INFO_MAX_ATTEMPTS", "2"), 1),
    3,
)
POLYMARKET_MARKET_INFO_RETRY_BACKOFF_SECONDS = _nonnegative_float_env(
    "POLYMARKET_MARKET_INFO_RETRY_BACKOFF_SECONDS",
    "0.5",
)
POLYMARKET_MARKET_INFO_TOTAL_BUDGET_SECONDS = max(
    _positive_float_env("POLYMARKET_MARKET_INFO_TOTAL_BUDGET_SECONDS", "25"),
    POLYMARKET_CLOB_READ_TIMEOUT_SECONDS,
)
...
BTC5M_POLL_SECONDS = max(_safe_float_env("BTC5M_POLL_SECONDS", "1"), 1.0)
BTC5M_REAL_TRADING_WINDOW_ENABLED = _is_truthy_env("BTC5M_REAL_TRADING_WINDOW_ENABLED", "1")
BTC5M_REAL_TRADING_WINDOW_TIMEZONE = (
    os.getenv("BTC5M_REAL_TRADING_WINDOW_TIMEZONE", "America/New_York").strip()
    or "America/New_York"
)
BTC5M_REAL_TRADING_START_HOUR = min(
    max(_safe_float_env("BTC5M_REAL_TRADING_START_HOUR", "9"), 0.0),
    24.0,
)
BTC5M_REAL_TRADING_END_HOUR = min(
    max(_safe_float_env("BTC5M_REAL_TRADING_END_HOUR", "17"), 0.0),
    24.0,
)
BTC5M_REAL_MIN_EXECUTABLE_PRICE = min(
    max(_safe_float_env("BTC5M_REAL_MIN_EXECUTABLE_PRICE", "0.60"), 0.0),
    0.99,
)
BTC5M_REAL_ORDER_SUBMIT_BUFFER_SECONDS = max(
    _safe_float_env("BTC5M_REAL_ORDER_SUBMIT_BUFFER_SECONDS", "2.0"),
    0.0,
)
...
for _asset_key, _asset_overrides in BTC5M_ALT_5M_ASSETS.items():
    ...
    BTC5M_PROFILES[f"{_asset_key}_late_capture_gap0025"] = {
        **BTC5M_PROFILES["late_capture_gap0025"],
        **BTC5M_ALT_5M_PROFILE_OVERRIDES,
        **_asset_overrides,
    }
    BTC5M_PROFILES[f"{_asset_key}_late_capture_gap0025_min88"] = {
        **BTC5M_PROFILES["late_capture_gap0025_min88"],
        **BTC5M_ALT_5M_PROFILE_OVERRIDES,
        **_asset_overrides,
    }
    BTC5M_PROFILES[f"{_asset_key}_late_capture_gap0025_last20"] = {
        **BTC5M_PROFILES["late_capture_gap0025_last20"],
        **BTC5M_ALT_5M_PROFILE_OVERRIDES,
        **_asset_overrides,
    }
    BTC5M_PROFILES[f"{_asset_key}_late_capture_gap0025_last10"] = {
        **BTC5M_PROFILES["late_capture_gap0025_last10"],
        **BTC5M_ALT_5M_PROFILE_OVERRIDES,
        **_asset_overrides,
    }
