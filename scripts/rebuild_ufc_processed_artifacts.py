...
def run_rebuild(
    *,
    ...
    update_production_manifest: bool = False,
) -> dict[str, object]:
    ...
    if update_production_manifest and any(subdir is None for subdir in output_dirs):
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Refresh the active runtime manifest when rebuilding the canonical data/processed snapshot.",
    )
    ...
    print(
        json.dumps(
            run_rebuild(
                ...
                update_production_manifest=args.update_production_manifest,
            ),
            indent=2,
        )
    )
