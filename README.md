...
- On Railway, the runtime source of truth is the reconciled production bundle manifest under the mounted data volume. As verified from the production `/readyz` response on 2026-07-10, the hosted service is running `full_live_contract_v6_durability_fullfit` from runtime bundle `ufc-production-20260627-full_live_contract_v6_durability_fullfit`, with both its manifest and processed snapshot current through 2026-06-27. The repository manifest in [models/current_production_model.json](models/current_production_model.json) records the original promoted model and 2026-06-06 snapshot; hosted bootstrap and refresh reconciliation preserve the same embedded model contract while rolling the persistent processed snapshot and its date-based runtime bundle ID forward. The entrypoint intentionally ignores legacy hosted overrides for `UFC_MODELS_DIR` and `UFC_PRODUCTION_BUNDLE_MANIFEST` so Railway does not accidentally load model artifacts from a stale volume.
...
| `BTC5M_MONITOR_ACTIVITY_ENRICHMENT_ENABLED` | External activity enrichment for the `/btc5m` dashboard | Optional; defaults to `1`. The dashboard automatically skips Polymarket activity/CLOB history enrichment while the crypto emergency stop is active, and setting this to `0` keeps the monitor local-ledger-only even when it is not paused |
...
- When the crypto emergency stop is active, `/api/btc5m/live` remains available but skips external Polymarket activity and CLOB trade-history enrichment so viewing a paused dashboard does not generate those upstream requests. Set `BTC5M_MONITOR_ACTIVITY_ENRICHMENT_ENABLED=0` to force the same local-ledger-only behavior without activating the emergency stop.
