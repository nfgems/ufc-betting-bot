...
- Real-money live trading checks processed-snapshot freshness against known completed UFC card dates when live UFC.com context is available. This avoids blocking late US cards solely because UTC has rolled into the next day, and it avoids blocking valid bets just because two UFC cards are 8+ calendar days apart. Outside the configured betting window this is a warning; once a matching fight is in-window, the bot still fails closed when the snapshot is missing a completed intervening card.
