...
| `TAPOLOGY_READER_BASE_URL` / `TAPOLOGY_READER_FALLBACK_ENABLED` | Reader-service Tapology fallback (preferred on Railway) | Optional; enabled by default (`TAPOLOGY_READER_FALLBACK_ENABLED=1`) using `TAPOLOGY_READER_BASE_URL` (defaults to `https://r.jina.ai/`). When a reader base URL is set and the process is running on Railway (detected via `RAILWAY_PROJECT_ID` / `RAILWAY_SERVICE_ID` / `RAILWAY_ENVIRONMENT`), Tapology profile and search pages that fail normal HTTP are fetched as markdown through the reader service first — ahead of the headed-browser fallback — and cached for the process. Set `TAPOLOGY_READER_FALLBACK_ENABLED=0` to disable. Advanced: `TAPOLOGY_READER_TIMEOUT_SECONDS` (default `45`, clamped to 20–180s) sets the reader request timeout |
