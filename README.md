...
- Before live trading, the runtime enforces a bundle-freshness guard: `predict` logs a warning and `live --real` is blocked when the promoted model is older than one month or the processed snapshot is more than 7 days behind the active UFC card date when live card context is available.
...
- Real-money live trading checks processed-snapshot freshness against the active UFC card date when live UFC.com context is available. This avoids blocking late US cards solely because UTC has rolled into the next day, while still failing closed when the snapshot is genuinely more than one card behind.
